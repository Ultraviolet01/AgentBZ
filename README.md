...
| **Agent discovery via directory** | ✅ **YES** | **Dynamic Agent Registry Discovery**:<br>• [`apps/api/src/routes/chat/orchestrate.ts#L103-L250`](https://github.com/Ultraviolet01/AgentBZ/blob/main/apps/api/src/routes/chat/orchestrate.ts#L103-L250) (Autonomous discovery engine scans the database registry, matches required tools, and selects candidate agents for execution). | Agent database registry integrated with live HCS-14 topic resolution and on-chain identity links. |
